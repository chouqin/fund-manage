...
def project_edit(request, project_id):
    ...
    teachers = project.teachers.all()
    projectTypes = ProjectType.objects.all()
    return render_to_response('project_edit.html', {'project': project, 'devices': devices, 'businesses': businesses, 'teachers': teachers, 'project_types': projectTypes})
...
def business_expense_add(request, business_id, teacher_id):
    ...
    if request.method == "POST":
        amount = request.POST['amount']
        business_expense = DeviceExpense(business=business, amount=amount, created_at=datetime.now(), status=1, teacher_id=teacher_id)
        business_expense.save()
        business.remain = business.remain - int(amount)
        business.save()
        return HttpResponseRedirect('/record')
    else:
